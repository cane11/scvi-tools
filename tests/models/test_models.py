...
def test_scanvi(save_path):
    ...
    model = SCANVI(adata, n_latent=10, n_version=0)
    ...
    model.train(1, train_size=0.5, check_val_every_n_epoch=1,  plan_kwargs=dict(mode="old"))

    model = SCANVI(adata, n_latent=10, n_version = 1)
    assert len(model._labeled_indices) == sum(adata.obs["labels"] != "label_0")
    assert len(model._unlabeled_indices) == sum(adata.obs["labels"] == "label_0")
    model.train(1, train_size=0.5, check_val_every_n_epoch=1,  plan_kwargs=dict(mode="new"))
